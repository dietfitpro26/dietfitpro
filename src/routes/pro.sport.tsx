...
function Content() {
  ...
  const load = async () => {
    ...
    const [{ data: progs }, { data: pats }] = await Promise.all([
      supabase
        .from("sport_programs")
        .select("id, name, patient_id, frequency_per_week, level, is_active, created_at")
        .eq("pro_id", user.id)
    ...
    setRows(((progs ?? []) as SportRow[]).map((r) => ({ ...r, patient: pmap.get(r.patient_id) ?? null })));
  };
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    setDeleting(true);
    const { error } = await supabase.from("sport_programs").delete().eq("id", deleteId);
    setDeleting(false);
  ...
  return (
    <div className="flex flex-col">
      <header className="flex items-center justify-between border-b bg-white px-6 py-4">
        <h1 className="text-xl font-semibold">Programmes sport</h1>
                ...
                : filtered.length === 0
                  ...
                  : filtered.map((r) => (
                    <TableRow key={r.id} className="cursor-pointer hover:bg-muted/40"
                      onClick={() => navigate({ to: "/pro/sport/$programId", params: { programId: r.id } })}>
                      <TableCell className="font-medium">{r.name}</TableCell>
                      <TableCell>{r.patient ? `${r.patient.first_name} ${r.patient.last_name}` : "—"}</TableCell>
                      <TableCell>{r.frequency_per_week ? `${r.frequency_per_week}×/sem.` : "—"}</TableCell>
                      <TableCell>{r.level ? LEVEL_LABEL[r.level] : "—"}</TableCell>
                      <TableCell>
...
function NewSportDialog({
  ...
  const submit = async (e: FormEvent) => {
    ...
    setSubmitting(true);
    const { error } = await supabase.from("sport_programs").insert({
      pro_id: user.id,
      patient_id: patientId,
      name: name.trim(),
      frequency_per_week: freq ? Number(freq) : null,
      level,
      goal: goal.trim() || null,
    ...
    setName(""); setPatientId(""); setFreq(""); setLevel("debutant"); setGoal(""); setNotes(""); setIsActive(true);
    onCreated();
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Nouveau programme sport</DialogTitle>
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <Label>Fréquence hebdo</Label>
              <Input type="number" min={1} max={14} value={freq} onChange={(e) => setFreq(e.target.value)} />
            </div>
            <div className="space-y-1">
              <Label>Niveau</Label>
          ...
          <div className="space-y-1">
            ...
            <Input value={goal} onChange={(e) => setGoal(e.target.value)} maxLength={200} />
          </div>
          <div className="space-y-1">
            <Label>Notes</Label>
