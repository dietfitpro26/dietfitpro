...
interface Props {
  // Données patient pré-remplies
  weight_kg: number | null;
  height_cm: number | null;
  age: number | null;
  gender: "homme" | "femme" | null;
  // Callback quand le pro valide
...
// ── Composant ──────────────────────────────────────────────
export function MacroCalculator({ weight_kg, height_cm, age, gender, onValidate }: Props) {
  ...
  const [fatG, setFatG]             = useState("30");
  ...
  // Recalcul auto si les inputs changent
  useEffect(() => {
    setCalculated(false);
  ...
  function handleCalculate() {
    ...
    const fec     = FECULENT_OPTIONS.find(f => f.value === feculent)!;

    const mbVal   = calcMB(w, h, a, sex);
    const tdeeVal = Math.round(mbVal * napVal);
    const kcal    = tdeeVal - deficit;

    // Protéines : 1.8g/kg arrondi
    const prot  = Math.round(w * 1.8);
    // Lipides fixes : 30g (10g × 3 repas)
    const fat   = 30;
    // Glucides : calories restantes
    const carbsKcal = kcal - (prot * 4) - (fat * 9);
    const carbs = Math.max(0, Math.round(carbsKcal / 4));

    // Féculents selon phase (cru)
    // Phase 1 : midi ~80g cru, soir ~40g cru
    // Phase 2 : midi ~50g cru, soir ~25g cru
    // Phase 3 : midi 0g, soir 0g
  ...
  function handleValidate() {
    ...
    onValidate({
      mb: mb!,
      tdee: tdee!,
      target_kcal:          parseInt(targetKcal),
      protein_g:            parseInt(proteinG),
      fat_g:                parseInt(fatG),
      carbs_g:              parseInt(carbsG),
      feculent_midi_cru_g:  parseInt(fMidiCru),
      feculent_soir_cru_g:  parseInt(fSoirCru),
      feculent_matin_pain_g: parseInt(painG),
      phase:                parseInt(phase) as 1 | 2 | 3,
    });
  }
      ...
      <CardContent className="space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div className="space-y-1">
              <Label>Âge *</Label>
              <Input type="number" value={ageVal}
                onChange={e => setAge(e.target.value)} />
            </div>
            <div className="space-y-1">
              <Label>Sexe *</Label>
              <Select value={sex} onValueChange={(v) => setSex(v as "homme" | "femme")}>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div className="space-y-1">
              ...
              <Select value={phase} onValueChange={(v) => setPhase(v as "1"|"2"|"3")}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {PHASE_OPTIONS.map(o => (
                    <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-muted/40 rounded-lg p-3 text-center">
                ...
                <p className="text-lg font-bold text-primary">{mb} <span className="text-xs font-normal">kcal</span></p>
              </div>
              <div className="bg-muted/40 rounded-lg p-3 text-center">
                <p className="text-xs text-muted-foreground">TDEE (dépense totale)</p>
                <p className="text-lg font-bold">{tdee} <span className="text-xs font-normal">kcal</span></p>
              </div>
              <div className="bg-muted/40 rounded-lg p-3 text-center">
                <p className="text-xs text-muted-foreground">Objectif calorique</p>
                <p className="text-lg font-bold text-green-600">{targetKcal} <span className="text-xs font-normal">kcal</span></p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="space-y-1">
                ...
                <Input type="number" value={targetKcal}
                  onChange={e => setTargetKcal(e.target.value)} />
              </div>
              <div className="space-y-1">
                <Label>🥩 Protéines (g)</Label>
                <Input type="number" value={proteinG}
              ...
              <div className="space-y-1">
                ...
                <Input type="number" value={fatG}
                  onChange={e => setFatG(e.target.value)} />
                <p className="text-xs text-muted-foreground">10g × 3 repas</p>
              </div>
              <div className="space-y-1">
                <Label>🌾 Glucides (g)</Label>
                <Input type="number" value={carbsG}
            ...
            {/* Féculents modifiables */}
            <div className="grid grid-cols-3 gap-3">
              <div className="space-y-1">
                <Label>🌅 Matin — Pain céréales</Label>
                <Input type="number" value={painG}
              ...
              <div className="space-y-1">
                ...
                <Input type="number" value={fMidiCru}
              ...
              <div className="space-y-1">
                ...
                <Input type="number" value={fSoirCru}
            ...
            <div className="bg-muted/30 rounded-lg p-4 space-y-2 text-sm">
              ...
              <p>☀️ <strong>Midi :</strong> Protéines + légumes à volonté + {fMidiCru ? `${fMidiCru}g cru (${+fMidiCru * 2}g cuit)` : "sans féculent"} {fecSelected.label} + 10g lipides crus</p>
              <p>🌙 <strong>Soir :</strong> Protéines + légumes à volonté + {fSoirCru ? `${fSoirCru}g cru (${+fSoirCru * 2}g cuit)` : "sans féculent"} {fecSelected.label} + 10g lipides crus</p>
            ...
            <div className="bg-primary/5 border border-primary/20 rounded-lg p-3 text-sm">
              ...
              <p className="font-bold mt-1">
                Total : {+proteinG * 4 + +fatG * 9 + +carbsG * 4} kcal
                {" "}
